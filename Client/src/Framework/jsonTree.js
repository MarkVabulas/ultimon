...
function _NodeComplex(label, val, isLast) {
    ...
    let childIsComplex = false;
    utils.forEachNode(val, function(label, node, isLast) {
        let newNode = Node(label, node, isLast);
        childIsComplex = childIsComplex || newNode.isComplex;
        self.addChild(newNode);
    });

    if (moreContentEl && !childIsComplex)
        moreContentEl.innerHTML = trimEnds(JSON.stringify(val));
    else
        self.el.classList.add('jsontree_node_expanded');
...
class Tree {
    ...
    loadData(jsonObj) {
        ...
        this.rootNode = Node('root', jsonObj, true);
